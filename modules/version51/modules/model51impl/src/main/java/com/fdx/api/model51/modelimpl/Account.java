...
import com.fdx.api.model51.mapper.AccountRowMapper;
...
@Repository("Account51")
public class Account<T> implements Account51<T> {
			...
			if (resultType.equals("details")) {
				try {
					if (accountid != null) {
						...
						descriptor = descriptor.getAccountDescriptor(accountid);
						if(account != null) {
							account.setContact((AccountContact) getAccountContact(accountid));
							account.setAccountDescriptor(descriptor);
						}
					}
	...
	public T getAccountContact(String accountid) throws FDXException {
		try {
			...
			if (accountid != null) {
				try {
					Account account = template.queryForObject("select * from " + DeveloperContext.getDeveloperSchema() + ".account where accountid in (:accountId)",
								parameters, new AccountRowMapper());
						if(account!=null) {
